...
**Gentoo had longer flippers on average. Adelie and Chinstrap were closely related**
...
**Jitterplots do a good job visualizing how the data is concentrated, boxplot's strength is showing how the quartiles are related**
...
**I like the jitter plot on top of the boxplot, since the boxplot covers a dense area and most of the data.**:
...
**mapping in gglot is global to all graphics, while mapping in one geom just applies to that plot**:
...
ggplot(data = penguins, mapping = aes(x = bill_length_mm, color = island)) +
  geom_histogram()
```

**It looks like species is what defines bill length the most. The Adelie group makes up most of the lower peak, while the other two species make up most of the higher peak. Sex has a pretty even spread across the data. Torgersen the island was mostly in the lower peak as well.**:
...
**Positive relationship between flipper length and body mass**:
...
**Gentoo and heavier with longer flippers, The other two are smaller with shorter flippers**:
...
**Transparency allows us to see density better**:
